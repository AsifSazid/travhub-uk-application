...
<body class="bg-gray-50 min-h-screen">
    ...
    <script>
        ...
        // Modify the generateFormSteps function to initialize date pickers
        function generateFormSteps() {
            ...
            // Initialize date picker after form is rendered
            setTimeout(() => {
                const inputId = `pp-dob-${state.currentApplicant}`;
                if (!window.datePickers[inputId]) {
                    window.datePickers[inputId] = new DatePicker(inputId, (isoDate, formattedDate) => {
                        updateApplicantData('passportInfo', 'pp_dob', isoDate);
                    });

                    // Set initial date if exists
                    const applicant = state.applicants[state.currentApplicant];
                    if (applicant.passportInfo.pp_dob) {
                        const date = new Date(applicant.passportInfo.pp_dob);
                        window.datePickers[inputId].selectedDate = date;
                    }
                }
            }, 100);
        ...
        // In generatePassportInfoStep() function, replace the Date of Birth field:
            ...
            return `
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="date-picker-with-btn">
                            <input type="text" 
                                id="pp-dob-${state.currentApplicant}" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 date-picker-input"
                                placeholder="Type: 1 Jan 2023 or click calendar button"
                                value="${applicant.passportInfo.pp_dob ? formatDisplayDate(new Date(applicant.passportInfo.pp_dob)) : ''}"
                                onchange="handleDateInputChange('${state.currentApplicant}', this.value)">
                            <button type="button" class="date-picker-btn" onclick="openDatePicker('${state.currentApplicant}')">
                                <i class="fas fa-calendar"></i>
                            </button>
                        </div>
                        <div id="pp-dob-${state.currentApplicant}-error" class="date-picker-error"></div>
                    </div>
                    <div>
